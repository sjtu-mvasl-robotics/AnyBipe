...
class SimEnvRobotTracker:
    def __init__(self, 
        ...
        self.feet_original_offset = [0.0, 0.203, 0.0]
        self.rs_t_current = 0
        self.rs_t_prev = 0
        self.rs_t = []
    ...
    def sensor_joy_cb(self, data: datatypes.SensorJoy):
        if str(data.buttons) == "L1 + X": # start moving
    ...
    def plot_velocity(self):
        ...
        ax[0].plot(self.imu_t, self.ref_vel_x * np.ones(len(self.imu_t)), label='ref')
        if falling_time_idx != -1:
            ax[0].axvline(x=self.imu_t[falling_time_idx], color='r', linestyle='--', label='falling time')
        ax[0].set_title("X Velocity")
        ax[0].legend()
        ax[1].plot(self.imu_t, self.imu_vel[1], label='imu')
        ax[1].plot(self.imu_t, self.ref_vel_y * np.ones(len(self.imu_t)), label='ref')
        ax[1].set_title("Y Velocity")
        if falling_time_idx != -1:
            ax[1].axvline(x=self.imu_t[falling_time_idx], color='r', linestyle='--', label='falling time')
        ax[1].legend()
        ax[2].plot(self.imu_t, self.imu_vel[2], label='imu')
        ax[2].plot(self.imu_t, np.zeros(len(self.imu_t)), label='ref')
        ax[2].set_title("Z Velocity")
        if falling_time_idx != -1:
            ax[2].axvline(x=self.imu_t[falling_time_idx], color='r', linestyle='--', label='falling time')

        ax[2].legend()
        plt.savefig(self.export_path + "imu_vel.png")
    ...
    def plot_acceleration(self):
        ...
        plt.savefig(self.export_path + "imu_acc.png")
    ...
    def plot_euler_angles(self):
        ...
        plt.savefig(self.export_path + "imu_ang.png")
    ...
    def run(self):
        ...
        sensor_joy_cb = partial(self.sensor_joy_cb)
        self.robot.subscribeSensorJoy(sensor_joy_cb)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ip_addr", type=str, default="10.192.1.2")
